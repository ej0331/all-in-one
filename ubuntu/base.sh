...
setup-basic-config() {
    ...
    if [ -f "ej0331.private.gpg" ]; then
        echo -e "${CYAN}${BOLD}CHECK ${BLUE}=> ${GREEN}GPG file exists!${CLEAR}"

        echo -e "\n${CYAN}${BOLD}STEP ${BLUE}=> ${WHITE}Import GPG key${CLEAR}"
        gpg --import ej0331.private.gpg

        echo -e "\n${CYAN}${BOLD}STEP ${BLUE}=> ${WHITE}Change trust level${CLEAR}"
        keyid=$(gpg --quiet --import-options import-show --import ej0331.private.gpg | sed -e '2!d' -e 's/^[ \t]*//') && echo "${keyid}:6:" | gpg --import-ownertrust
